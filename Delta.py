#!/usr/bin/env python
# coding: utf-8

# # Creates Figure 7

# In[6]:


import netCDF4 as nc
import numpy as np
import xarray as xr
import datetime
from datetime import timedelta
import xesmf as xe
import cftime

import matplotlib.pyplot as plt
import cartopy
import cartopy.crs as ccrs
import cartopy.feature as cfeature


# In[7]:


#loading data
mon_anom = xr.open_dataset('/gpfs/group/cmz5202/default/ros/CESM_LENS_deltas/T/ens_T_anom.nc')
mon_avg = xr.open_dataset('/gpfs/group/cmz5202/default/ros/CESM_LENS_deltas/T/ens_mean.nc')


# In[8]:


#creating array of letters for subplot labels
letters = ['(a)','(b)','(c)','(d)','(e)','(f)','(g)']


# In[9]:


## making figure to show delta technique process

#setting up plot
fig, axs = plt.subplots(1,3,subplot_kw={'projection': ccrs.PlateCarree()},figsize=(20,7),constrained_layout=True)
#choosing time indices 
t1 = 911  #corresponds to jan 1996
t2 = 1931 #corresponds to jan 2081 (+4K)

#loop to plot three subplots of two different years and the difference of the two
for ax,i,t,L in zip(axs,[t2,t1,0],['2081 January Surface Temperature','1996 January Surface Temperature','Delta: January Surface Temperature'],letters):
    #plotting the difference
    if i == 0:
        cont = np.arange(0,31,1)
        plot = ax.contourf(mon_avg['lon'],mon_avg['lat'],mon_avg['T'][t2,29,:,:] - mon_avg['T'][t1,29,:,:], levels=cont,cmap='YlOrRd')
        #adding an = between the last two subplots
        ax.text(-0.08,.5,'=',ha='center',va='center',fontsize=40,fontweight='bold',transform=ax.transAxes)
    #plotting the two years
    else:
        cont = np.arange(229,318,4)
        plot = ax.contourf(mon_avg['lon'],mon_avg['lat'],mon_avg['T'][i,29,:,:], levels=cont,cmap='seismic')
        #adding a subtraction sign between the first two subplots
        if i == t1:
            ax.text(-0.08,.5,'-',ha='center',va='center',fontsize=60,fontweight='bold',transform=ax.transAxes)
    #adding letters to panels
    props = dict(boxstyle='square', facecolor='white', edgecolor='k')
    ax.text(.02,.97,L, fontsize='xx-large',transform=ax.transAxes,ha='left',va='top',bbox=props)
    #adding title
    ax.set_title(t,fontsize=24,ha='center')
    #adding features to plot
    ax.add_feature(cfeature.BORDERS)
    ax.add_feature(cfeature.COASTLINE)
    ax.add_feature(cfeature.STATES)
    #adding colorbar to bottom of subplots
    cb = plt.colorbar(plot, ax=ax, shrink=.95,aspect=8,location='bottom')
    cb.ax.tick_params(labelsize='xx-large')
    cb.ax.set_title('K', fontsize='xx-large')

fig.savefig('delta_example.pdf', bbox_inches='tight', pad_inches=0)


# In[ ]:




